# Hi, I'm Evan Ferron! 👋

### Tech Enthusiast & Developer 🇫🇷

Welcome to my profile! I am a developer based in France, naturally curious and always seeking new technical challenges. I love building high-performance, robust, and elegant applications.

---

### 🛠 My Favorite Tech Stack

These are the tools I enjoy working with the most:

*   **Frontend:** ![React](https://img.shields.io/badge/React-20232A?style=flat&logo=react&logoColor=61DAFB) ![TypeScript](https://img.shields.io/badge/TypeScript-007ACC?style=flat&logo=typescript&logoColor=white)
*   **Backend & Systems:** ![Rust](https://img.shields.io/badge/Rust-000000?style=flat&logo=rust&logoColor=white)

### 💼 Professional Experience & Tools

Technologies and tools I use in a professional environment:

*   **Languages:** ![Java](https://img.shields.io/badge/Java-ED8B00?style=flat&logo=openjdk&logoColor=white) ![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
*   **DevOps & Infra:** ![Kubernetes](https://img.shields.io/badge/Kubernetes-326CE5?style=flat&logo=kubernetes&logoColor=white) ![GitLab](https://img.shields.io/badge/GitLab-FC6D26?style=flat&logo=gitlab&logoColor=white)
*   **Agile & Management:** ![Jira](https://img.shields.io/badge/Jira-0052CC?style=flat&logo=jira&logoColor=white)

---

### 🚀 What I'm Up To
- 🦀 Deepening my **Rust** skills through advanced projects (ETL, Database Manager).
- ⚛️ Building reactive interfaces with **React** and **TypeScript**.
- ✅ **Professional Hygiene:** For every project, I implement industry-standard best practices, including thorough testing, strict branching strategies, and clear project scoping/documentation.

---

### 🤝 Get in Touch
- 💼 [LinkedIn](https://www.linkedin.com/in/evan-ferron-2b6a9a254)
- 📧 [evan.ferron53@gmail.com](mailto:evan.ferron53@gmail.com)
